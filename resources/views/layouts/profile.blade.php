...
<head>
    ...
    <!-- Favicons -->
    <link href="{{ asset('image/icon/' . $contact['logo']) }}" rel="icon">
    <link href="{{ asset('image/icon/' . $contact['logo']) }}" rel="apple-touch-icon">
  ...
  <section id="topbar" class="topbar d-flex align-items-center " >
    <div class="container d-flex justify-content-center justify-content-md-between">
      <div class="contact-info d-flex align-items-center">
        ...
        <i class="bi bi-phone d-flex align-items-center ms-4"><span >+ {{ $contact['no_whatsapp'] }}</span></i>
      </div>
      <div class="social-links d-none d-md-flex align-items-center">
        <a href="{{ 'https://twitter.com/' . $contact['twitter'] . '/' }}" class="twitter" ><i class="bi bi-twitter"></i></a>
        <a href="{{ 'https://www.facebook.com/' . $contact['facebook'] . '/' }}" class="facebook" ><i class="bi bi-facebook"></i></a>
        <a href="{{ 'https://www.instagram.com/' . $contact['instagram'] . '/' }}" class="instagram" ><i class="bi bi-instagram"></i></a>
        <a href="{{ $contact['youtube'] }}" class="youtube" ><i class="bi bi-youtube"></i></i></a>
      </div>
    </div>
    ...
    <header id="header" class="header d-flex align-items-center">
        ...
        <div class="container-fluid container-xl d-flex align-items-center justify-content-between">
            <a href="{{ route('/') }}" class="logo d-flex align-items-center">
                ...
                <img src="{{ asset('image/icon/' . $contact['logo']) }}" alt="">
                        ...
                        @if ($nav['dropdown'] == 'true')
                            <li class="dropdown">
                                <a href="#" @if (Route::currentRouteName() == 'detail' || Route::currentRouteName() == 'project') class="active" @endif>
                                    <span>{{ $nav['name'] }}</span> <i
                                        class="bi bi-chevron-down dropdown-indicator"></i>
                                </a>
                                <ul>
                                    @foreach ($nav['dropdown_list'] as $sub_menu)
                                        <li><a
                                                href="{{ route($nav['route'], $sub_menu['id_project']) }}">{{ $sub_menu['project_name'] }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </li>
                        @else
                            <li><a href="{{ route($nav['url']) }}"
                                    @if (Route::currentRouteName() == $nav['url']) class="active" @endif>{{ $nav['name'] }}</a>
                            </li>
                        @endif
                    @endforeach
            ...
            <div class="row gy-4">
                <div class="col-lg-5 col-md-12 footer-info">
                    ...
                    <div class="social-links d-flex mt-4">
                        <a href="{{ 'https://twitter.com/' . $contact['twitter'] . '/' }}" class="twitter"><i
                                class="bi bi-twitter"></i></a>
                        <a href="{{ 'https://www.facebook.com/' . $contact['facebook'] . '/' }}" class="facebook"><i
                                class="bi bi-facebook"></i></a>
                        <a href="{{ 'https://www.instagram.com/' . $contact['instagram'] . '/' }}" class="instagram"><i
                                class="bi bi-instagram"></i></a>
                        <a href="{{ $contact['youtube'] }}" class="youtube"><i class="bi bi-youtube"></i></a>
                    </div>
                </div>
            ...
            <li><a href="#">Graphic Design</a></li>
                ...
                <div class="col-lg-4 col-md-12 footer-contact text-center text-md-start">
                    ...
                    <p>
                        ...
                        <strong>Whatsapp:</strong> + {{ $contact['no_whatsapp'] }}<br>
